...
cultCard=h('div',{class:'card'},[h('h3',{text:'洞府'}),h('div',{class:'sub',text:(d.role||'散修')+(d.sectName?' · '+d.sectName:'')+' · 今日悬赏 '+bq.done+'/'+bq.total}),h('div',{class:'kv'},[h('b',{text:'灵根'}),h('span',{text:rootLabel(m.root)}),h('b',{text:'功法'}),h('span',{text:'《'+d.gf.name+'》'+['黄','玄','地','天'][d.gf.grade]+'阶'}),h('b',{text:'修炼'}),h('span',{text:fmt(st.ratePerHour)+' 修为/时（'+m.realm.slice(0,2)+'基础 '+fmt(Math.round(st.ratePerHour/st.rate))+' × '+st.rate.toFixed(2)+'，已乘）· 离线上限 '+d.capHours+' 小时'}),h('b',{text:'寿元'}),h('span',{text:m.age+' / '+m.life+' 岁'+(m.life-m.age<30?'  ⚠ 大限将至':'')}),dbfs.length?h('b',{text:'状态'}):null,dbfs.length?h('span',{},[dbfs.map(function(x){return x.n+' 余 '+dur(m.dbf[x.k]-m.now)+'（'+x.e+'）'}).join('　'),h('div',{class:'muted',text:'到点自动消，养神丹可立刻尽去。'})]):null,m.buffs&&m.buffs.length?h('b',{text:'丹效'}):null,m.buffs&&m.buffs.length?h('span',{text:m.buffs.map(function(b){return b.n+' ×'+b.m.toFixed(2)}).join('、')}):null]),h('div',{class:'row',style:'margin-top:10px'},[breath,btBtn,bqBtn]),h('div',{class:'muted',text:'吐纳：每 10 分钟一次，每日 30 次。突破失败会走火入魔；大境界需渡劫。'+(d.btStreak?'  已连败 '+d.btStreak+' 次，下次成功率 +'+(d.btStreak*10)+'%（保底）。':'')})]);kids.push(cultCard);if(d.farm){var ftc=tipCard('farm');if(ftc)kids.push(ftc)}kids.push(farmCard(d.farm));
// v48 活动横幅 + 师徒
...
function sealBtn(x,d){if(x.equipped||x.basic)return null;return h('button',{class:'sm sealb',onclick:async function(ev){ev.stopPropagation();if(!(await sure('把《'+x.name+'》封存成册？自己就不会了，秘籍可上拍。')))return;var r=await rpc('book.seal',{id:x.id});if(r&&r.ok)renderBag(Object.assign({},d,r.data),'skills')}},'封存')}
...
function aucChips(d){var af=S.aucF||{};function chip(on,txt,fn){return h('button',{class:'sm'+(on?' pri':''),onclick:function(){fn();S.aucF=af;renderMarket(d)}},txt)}var ts=[chip(af.t==null,'全部',function(){delete af.t})].concat(TIERN.map(function(n,i){return chip(af.t===i,n,function(){af.t=i})}));var ks=[['','全部'],['art','法宝'],['pill','丹药'],['mat','材料'],['book','典籍'],['tal','符箓'],['egg','兽卵'],['misc','杂物']].map(function(k){return chip((af.k||'')===k[0],k[1],function(){af.k=k[0]})});return h('div',{},[h('div',{class:'row',style:'gap:4px;flex-wrap:wrap;margin:4px 0'},[h('span',{class:'muted',text:'品阶'})].concat(ts)),h('div',{class:'row',style:'gap:4px;flex-wrap:wrap;margin:4px 0 6px'},[h('span',{class:'muted',text:'种类'})].concat(ks))])}
...
var af=S.aucF||{};var aucList=a.open.filter(function(x){return (af.t==null||(x.item.t|0)===af.t)&&(!af.k||(x.item.k==='art'?'art':x.item.k)===af.k)});var esc=Object.keys(a.escrow||{}).reduce(function(t,k){return t+(Number(a.escrow[k])||0)},0);var auc=h('div',{class:'card'},[h('h3',{text:'拍卖行'}),h('div',{class:'row sb'},[h('span',{class:'sub',text:'出价托管灵石，落槌由天道裁定，回来即自动结算'}),h('span',{class:'num',text:'囊中 ◆ '+fmt(m.ls)+(esc?'　托管 ◆ '+fmt(esc):'')})]),h('h4',{text:'在拍（'+(a.openTotal||a.open.length)+(a.openTotal>a.open.length?'，先列最先落槌的 '+a.open.length+' 件':'')+'）'}),aucChips(d),h('div',{class:'list'},aucList.length?aucList.map(function(x){return h('div',{class:'row sb'},[h('span',{class:'row grow'},[img('item_'+x.item.id,'ico sm'),tierTag(x.item.t||0),h('span',{},[itemName(x.item)+' · '+x.seller+' · 余 '+left(x.end),h('div',{class:'muted',text:itemAttr(x.item)})])]),h('span',{class:'row'},[h('span',{class:'num',text:(x.top?'现价 '+x.top.amt:'起拍 '+x.min)+' '}),x.myBid?h('span',{class:'tag '+(x.top&&x.top.amt>x.myBid?'red':'gold'),text:'我出 '+x.myBid+(x.top&&x.top.amt>x.myBid?' · 已被超':' · 领先')}):null,h('button',{class:'sm pri',onclick:async function(){var amt=await ask('出价（不低于 '+(x.top?Math.ceil(x.top.amt*1.05):x.min)+'）',String(x.top?Math.ceil(x.top.amt*1.05):x.min),{type:'number'});if(!amt)return;var r=await rpc('auction.bid',{aid:x.aid,amt:Number(amt)});if(r&&r.ok)renderMarket(Object.assign({},d,r.data))}},'出价')])])}):h('div',{class:'muted',text:a.open.length?'这一类暂时没有在拍的。':'空空如也。去行囊里把东西挂上来。'})),a.mine.length?h('h4',{text:'我的拍品'}):null,h('div',{class:'list'},a.mine.map(function(x){return h('div',{text:itemName(x.item)+' · '+(x.settled?(x.settled.winner?'成交 '+x.settled.price+'（'+x.settled.wname+'）':'流拍'):x.ended?'落槌，等待裁定':'在拍 余 '+left(x.end)+(x.top?' 现价 '+x.top.amt:' 无人出价'))+(x.claimed?' ✓':'')})})),a.ended.length?h('h4',{text:'已落槌（我参与的）'}):null,h('div',{class:'list'},a.ended.map(function(x){return h('div',{text:itemName(x.item)+' · '+(x.settled?(String(x.settled.winner)===String(m.uid)?'你拍得 '+x.settled.price:'他人拍得 '+x.settled.price):'等待裁定')})}))]);
